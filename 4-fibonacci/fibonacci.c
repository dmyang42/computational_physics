...
#include <string.h>
#include <stdarg.h>
#define N 100000   // the amount of random number to generate
#define p 250
#define q 103
#define START "START"
#define END "END"
...
int schrage_generate (int seed) {
	 ...
	 * r = m mod a 
	 */
	int a = 16807, b = 0, m = 2147483647, t = 127773, r = 2836;
	int temp;
	int z = seed;
    random_numbers[0] = z;

	for (int i = 1; i < p; ++i) {
		temp = a * (z % t) - r * (int)(z / t);
		if (temp >= 0)
			z = temp;
		else
			z = temp + m;
...
int fibonacci_generate (int seed) {
    int m = 2147483647; int z;
    schrage_generate(seed);

    for (int i = p; i < N; ++i) {
        z = (int)(random_numbers[i - p] - random_numbers[i - q]) % m;
        if (z > 0) random_numbers[i] = z;
		else random_numbers[i] = z + m;
    }
    for (int i = 0; i < N; ++i)
